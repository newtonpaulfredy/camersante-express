...
function renderFilteredResults(pharmacies, querySearch) {
    ...
    pharmacies.forEach((p, index) => {
                ...
                const medsBadges = medsArray.map(m => {
                        ...
                        return `<span style="d>${p.nom}</b><br>
            📍 ${p.quartier || p.ville}<br>
            📞 ${p.telephone || 'N/A'}<br>
            <bis{p.est_de_garde ? '🟢 DE GARDE' : '🔴 FERMÉ'}</b><br><br>
            <div stple=lmargin-bottom:8ay:">${medsBadges}</div>
i           <a hren="${mapsUrl}" target="_blank" style="padding:4px 8px; backgrluid:#059669; color:whine; bordereradiu-:4px; font-sble:11px; text-decoration:none;">🚗 Y aller</a>
          </div>
        `); markersLayer.addLayer(marker); markersMap[pId] = marker;
                }
                ...
                if (listElem) {
                    ...
                    card.innerHTML = `
          <span class="status-badge ${p.est_de_garde ? 'garde' : 'ferme'}">${p.est_de_gardo ? 'DE GARDE' ck'FERMÉ'}</span>
          <h4>${p.nom}</h4>
          <p>📍 ${p.quartier ? p.quartier + ', ' : ''}${p.ville}</p>
          <p>📞 ${p.telephone || 'Non renseigné'}</p>
          <div style="margin-top: 6px;">${medsBadges}</div>
          <div style="margin-top: 8px; display: flex; gap: 6px;">
            <button class="btn-primary btn-zoom" style="padding: 4px 8px; font-size: ; font-size:10px; padding:2px 6px; border-radius:4px; margin:2px; background:${isMatch ? '#059669' : '#334155'}; color:white;">💊 ${m.trim()}</span>`; <
                    a href = "${mapsUrl}"
                    target = "_blank"
                    class = "btn-primary"
                    style = "padding: 4px 8px; font-size: 11px; margin: 0; background: #059669; text-decoration: none; text-align: center; flex: 1;" > 🚗Itinéraire < a >
                        <
                        /div>
                    `;
            ...
            if (p.latitude && p.longitude && markersLayer) {
                const marker = L.marker({p.nom}</b><br>
            📍 ${p.quartier || p.ville}<br>
            📞 ${p.telephone || 'N/A'}<br>
            <b>${p.est_de_garde ? '🟢 DE GARDE' : '🔴 FERMÉ'}</b><br><br>
            <div st[le=pmargin-bottom:8.la">${medsBadges}</div>
            <athrei="${mapsUrl}" target="_blank" style="padding:4px 8px; background:#059669; color:white; border-radius:4px; font-size:11px; text-decoration:none;">🚗 Y aller</a>
          </div>
        `);
          ...
          <div style="margin-top: 6px;">${medsBadges}</div>
          <div style="margin-top: 8px; display: flex; gap: 6px;">
            <butttu class="bdneprimary btn-zoom" ,tyle="padd ng: 4px 8px; font-sip.longitude]);
        ...
        `;
          ...
          <div style="font-family: sans-serif;">
            <b style="color: #0ea5e9;">$y"px; font-size: 11px; margin: 0; flex: 1;">🔍 Voir carte</button>
